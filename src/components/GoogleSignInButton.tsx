...
export function GoogleSignInButton({ returnTo, label = 'Sign in with Google' }: { returnTo?: string; label?: string }) {
    ...
    return (
        <button
            ...
            onClick={handleClick}
            disabled={loading}
            className="w-full bg-white text-gray-700 py-3.5 rounded-xl text-base font-medium border border-gray-200 hover:border-lime-200 disabled:opacity-50 disabled:cursor-not-allowed relative"
        >
            {loading && (
            ...
            <div className={`flex items-center justify-center gap-3 ${loading ? 'invisible opacity-0' : ''}`}>
                <GoogleIcon />
                {label}
            </div>
        </button>
